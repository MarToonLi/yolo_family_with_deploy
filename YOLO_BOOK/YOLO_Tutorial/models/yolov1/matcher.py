...
class YoloMatcher(object):
    ...
    def __call__(self, fmp_size, stride, targets):
        ...
        for batch_index in range(bs):
            targets_per_image = targets[batch_index]              # 每张图像的标签数据
            # [N,]
            tgt_cls = targets_per_image["labels"].numpy()         # 每张图像的类别标签
            # [N, 4]
            tgt_box = targets_per_image['boxes'].numpy()          # 每张图像的box标签数据
            ...
            for gt_box, gt_label in zip(tgt_box, tgt_cls):
                ...
                if grid_x < fmp_w and grid_y < fmp_h:
                    # 标记objectness标签，即此处的网格有物体，对应一个正样本: gt_objectness: [B, fmp_W, fmp_h]
                    #! 根据真实的标签数据，获得batch中每个图像在哪个网格位置处存在物体
                    gt_objectness[batch_index, grid_y, grid_x] = 1.0

                    # 标记正样本处的类别标签，采用one-hot格式: gt_classes [B, fmp_W, fmp_h, num_classes]
                    cls_ont_hot = np.zeros(self.num_classes)
                    cls_ont_hot[int(gt_label)] = 1.0
                    gt_classes[batch_index, grid_y, grid_x] = cls_ont_hot

                    #! 标记正样本处的bbox标签: gt_bboxes [B, fmp_w, fmp_h, 4]
