...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if RANK in {-1, 0}:                                                             # todo:  RANK 如果等于0或者-1代表什么
        ...
        for k in methods(loggers):
            callbacks.register_action(k, callback=getattr(loggers, k))  # 好工程组织方式：回调函数本是loggers的方法
...
def parse_opt(known=False):
    ...
    parser.add_argument('--evolve',          type=int, nargs='?', const=30,                            help='[trick] evolve hyperparameters for x generations')  # ? 使用超参数优化算法进行自动调参
...
def main(opt, callbacks=Callbacks()):
    ...
    LOGGER.debug(f"opt.hyp2: {opt.hyp}")
    ...
    # Evolve hyperparameters (optional)
        ...
        meta = {
            'lr0': (1, 1e-5, 1e-2),             # initial learning rate (SGD=1E-2, Adam=1E-3)
            ...
            'degrees': (1, 3.0, 45.0),       # image rotation (+/- deg)
            'translate': (1, 0.1, 0.9),      # image translation (+/- fraction)
            'scale': (1, 0.1, 0.9),          # image scale (+/- gain)
            'shear': (1, 0.1, 10.0),         # image shear (+/- deg)
            'perspective': (0, 0.0, 0.001),  # image perspective (+/- fraction), range 0-0.001
            'flipud': (1, 0.1, 1.0),         # image flip up-down (probability)
            'fliplr': (1, 0.1, 1.0),         # image flip left-right (probability)
            'mosaic': (1, 0.1, 1.0),         # image mixup (probability)
